    ...
    create({ template , types : t } ,{opts}){ 
        return { 
            ...
            MemberExpression(nodePath ,{opts ,file}){
                ...
                const { type , parentType , left ,context , right } = parseExpression ;  
                        ...
                        switch(objectName[0]){
                            case "module" : 
                                if(objectName[1] === 'exports'){  
                                    if(index === 2 && typeof prepare['module.exports'] === 'function'){
                                        const parser = prepare['module.exports'].call(nodePath, objectName,index ,right);
                                        if(typeof parser === "object"){
                                            const  { spliter , value , right} = parser ;
                                            if(typeof spliter === "number"){
                                                var propertyNames = objectName.slice(spliter);
                                                nodePath.parent.right = right || nodePath.parent.right;
                                                nodePath.replaceWith(
                                                    t.identifier(
                                                        [value , ...propertyNames ].filter(i => i).join('.')
                                                    )
                                                )
                                            } 
                                        }
                            ...
                            case "exports" :
                                if(index >= 2 && typeof prepare['exports.*'] === 'function'){
                                    const parser = prepare['exports.*'].call(nodePath,objectName,index,right);
                                    if(typeof parser === "object"){
                                        const  { spliter , value , right} = parser ;
                                        if(typeof spliter === "number"){
                                            var propertyNames = objectName.slice(spliter);
